...
export default {
  ...
  methods:{
    getMyOrder() {
      let {page,limit}=this
      this.$store.dispatch('getMyOrder',{page, limit})
    },
    getPageNo(pageNo){
      this.page=pageNo
      this.getMyOrder()
  ...
  computed:{
    ...mapState({
      myOrderInfo:state=>state.pay.myOrderInfo,
    })
  }
}
</script>
  ...
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        ...
        <table class="order-item" v-for="(record,index) in myOrderInfo.records" :key="index">
          <thead>
          <tr>
            <th colspan="5">
          ...
          <tbody>
          <tr v-for="(cart,index) in record.orderDetailList" :key="index">
            <td width="60%">
              <div class="typographic">
                <img :src="cart.imgUrl" style="width: 80px;height: 80px;">
                <a href="#" class="block-text">{{cart.skuName}}</a>
                <span>x{{cart.skuNum}}</span>
                <a href="#" class="service">售后申请{{myOrderInfo.current}}</a>
              </div>
            </td>
