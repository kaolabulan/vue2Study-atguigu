          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spu,index) in spuSaleAttrList" :key="spu.id">
                ...
                <dd changepirce="0" :class="{active:isChecked(spu2)}"
                    v-for="(spu2,index) in spu.spuSaleAttrValueList"
                    :key="spu2.id" @click="changeIsisChecked(spu2,spu.spuSaleAttrValueList)">{{
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" :value="skuNum" @change="changeSkuNum">
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:1">-</a>
              </div>
              <div class="add">
                <a @click="addShopCat">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    isChecked(spuSaleAttrValue){
      if(spuSaleAttrValue.isChecked==='1'){
        if (this.ISCHECKED.indexOf(spuSaleAttrValue)===-1) {
          let index = -1
          //遍历查重 重复返回索引index 不重复index=-1
          for (let ele of this.ISCHECKED){
            if (spuSaleAttrValue.baseSaleAttrId===ele.baseSaleAttrId){
              index = this.ISCHECKED.indexOf(ele)
            }
          }
          //初始化 在数组没有内容或不重复时直接添加  重复时splice
          if (this.ISCHECKED.length===0||index===-1) {
            this.ISCHECKED.push(spuSaleAttrValue)
          } else{
            this.ISCHECKED.splice(index,1,spuSaleAttrValue)
          }
        }
        return true
      }else {
    ...
    changeIsisChecked(spu2, arrSpu2) {
      arrSpu2.forEach(item => {
        item.isChecked = '0'
      })
      spu2.isChecked = '1'
    },
    ...
    async addShopCat() {
      //try{?}catch(error){??} 执行try报错执行catch
      try {
        //addOrUpdateShopCart返回的是一个Promise 有一个属性PromiseResult是它的返回结果
        //加上 async+await 组合 可以是返回结果为PromiseResult
